...
#filter, map, reduce, hiher order functions, list comprehension , annonymous functions
...
# def PieceLocation(X_click, Y_click):    
#     X = None
#     Y = None
#     for i in range(len(XStart)):

#         if X_click > XStart[i] and X_click < XEnd[i]:
#             X = Game_CordX[i]
#         if Y_click > YStart[i] and Y_click < YEnd[i]:
#             Y = Game_CordY[i]
#     return X, Y
...
def getDiagonalDec(board, digNum):
    if digNum <= len(board) - 1:
        ...
        # index = len(loa) - 1
        # for (i, j) in enumerate(range(digNum, -1, -1)):
        #     lst.append(board[j][index - i])
        # return lst

        # lst = []
        # for (i, j) in enumerate(range(digNum, -1, -1)):
        #     lst.append(board[j][len(board) - 1 - i])
        # return lst

        # lst = [board[j][len(board) - 1 - i] for (i, j) in enumerate(range(digNum, -1, -1))]
        # return lst

        return [board[j][len(board) - 1 - i] for (i, j) in enumerate(range(digNum, -1, -1))]
    else:
    #     index = (len(board) * 2 - 2) - digNum
    #     for i in range(len(board) - 1, digNum - len(board), -1):
